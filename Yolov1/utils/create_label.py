...
class CreateTargets():
    ...
    def build_target_without_anchors(self, target):
        ...
        xc_s = nx * self.image_size / self.strides[stride_ind]
        yc_s = ny * self.image_size / self.strides[stride_ind]
        grid_x = xc_s.long()
        grid_y = yc_s.long()

        return [stride_ind, batch_ind, anchor_ind, grid_x, grid_y]
    ...
    def create_targets(self, center_sample=False):
        ...
        for s in self.strides:
            fmp_w = self.image_size / s
            fmp_h = self.image_size / s
            y_trues.append(torch.zeros([num_of_batch_ind, num_anchor_of_one_layer, fmp_h, fmp_w, 1+1+4+1]))

        for i, label in enumerate(self.labels):
            cls_ind = label[1]
            # 平衡因子box_scale: 小框给大权重，大框给小权重
            box_scale = 2.0 - (label[4] / self.image_size) * (label[5] / self.image_size)
            position_target = []
...
if __name__ == "__main__":
    ...
    create_targets = CreateTargets(image_size=(640, 640), anchors=anchors, strides=strides, labels=None)
